...
bot.onText(/\/stats/, async (msg) => {
  ...
  try {
    ...
    const now = new Date();
    let activeSubs = 0, totalSubs = 0, giftSubs = 0, paidSubs = 0;
    let totalTasks = 0;
    ...
    // Считаем задачи из users/{id}/tasks
...
// ============ ПРОВЕРКА НАПОМИНАНИЙ (читаем из users/{userId}/tasks) ============
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      try {
        ...
        for (const documentSnapshot of snapshot.docs) {
          ...
          const reminderAt = task.reminderAt.toDate
            ? task.reminderAt.toDate()
          ...
          try {
            if (task.status === "done") {
              await updateDoc(
                doc(db, "users", userId, "tasks", documentSnapshot.id),
                { isSent: true }
              );
              continue;
            }
            ...
            if (task.repeat === "daily" && task.status !== "done") {
              await createNextDailyTask(task, userId);
            }

            console.log(`✅ Уведомление: ${userId} — ${task.title}`);
          } catch (err) {
        ...
        console.log(`Ошибка для пользователя ${userId}: ${err.message}`);
      }
    }
  } catch (err) {
...
async function createNextDailyTask(task, userId) {
  try {
    const reminderAt = task.reminderAt.toDate
      ? task.reminderAt.toDate()
      : new Date(task.reminderAt);

    const nextDate = new Date(reminderAt);
    ...
    const newTaskId = `daily_${Date.now()}`;
    await setDoc(doc(db, "users", userId, "tasks", newTaskId), {
      id: newTaskId,
      userId,
      title: task.title,
...
setInterval(cleanupOldDoneTasks, 6 * 60 * 60 * 1000);  { command: "myid", description: "Узнать свой ID" },
]);
