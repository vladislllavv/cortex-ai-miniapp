  ...
  return (
    <div style={{ paddingTop: "8px", paddingBottom: "20px" }}>
      ...
      <div style={{
          ...
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "12px" }}>
              {[
                ...
                ru ? "Безлимитный AI (вместо 5)" : "Unlimited AI (instead of 5)",
                ru ? "Дни рождения в облаке" : "Birthdays in cloud",
                ru ? "Уведомления о ДР и событиях" : "Birthday & event notifications",
              ].map((f) => (
        ...
        {/* Кнопка подписки — всегда видна */}
        <button
          onClick={openSubscribe}
          style={{
            width: "100%", height: "44px", borderRadius: "12px", border: "none",
            backgroundColor: subInfo.isActive ? "rgba(34,197,94,0.15)" : "#3b82f6",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "20px" }}>
        {[{ code: "ru", label: "🇷🇺 Русский" }, { code: "en", label: "🇺🇸 English" }].map(({ code, label }) => (
          <button key={code} onClick={() => setLanguage(code as "ru" | "en")} style={{
            ...
            color: language === code ? "#60a5fa" : "rgba(255,255,255,0.6)", cursor: "pointer",
          }}>
            {label}
          </button>
        ))}
      ...
      <button
        onClick={() => {
          const tg = (window as any).Telegram?.WebApp;
          tg?.showConfirm(ru ? "Удалить все задачи?" : "Delete all tasks?", (confirmed: boolean) => {
      ...
      {showAddCategory && (
        ...
        >
          <div
            ref={modalRef}
            style={{ backgroundColor: "#1e293b", borderRadius: "20px", padding: "20px", width: "100%", maxWidth: "320px", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "80vh", overflowY: "auto" }}
          ...
          >
            ...
            <div style={{ marginBottom: "12px" }}>
              <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", margin: "0 0 6px 0" }}>{ru ? "Иконка" : "Icon"}</p>
            ...
            <div style={{ marginBottom: "16px" }}>
              <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", margin: "0 0 6px 0" }}>{ru ? "Цвет" : "Color"}</p>
