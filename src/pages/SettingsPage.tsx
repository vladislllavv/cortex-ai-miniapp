...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (id && id !== "unknown") {
      getDoc(doc(db, "users", id, "integrations", "googleCalendar"))
        .then(snap => { if (snap.exists()) setGcalConnected(snap.data()?.connected === true); })
        .catch(() => {});
    }
  }, []);
  ...
  return (
    <div style={{ paddingTop: "8px", paddingBottom: "20px" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "10px" }}>
          <div style={{
            ...
            backgroundColor: gcalConnected ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.08)",
            border: gcalConnected ? "1px solid rgba(34,197,94,0.4)" : "1px solid rgba(255,255,255,0.1)",
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: "14px", fontWeight: 600, color: "white", margin: 0 }}>
              {gcalConnected ? (ru ? "Подключён ✅" : "Connected ✅") : (ru ? "Не подключён" : "Not connected")}
            </p>
            <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)", margin: 0 }}>
              {ru ? "Задачи с дедлайном → Google Calendar" : "Tasks with deadline → Google Calendar"}
            </p>
          </div>
        </div>
        {gcalConnected ? (
          <button
            onClick={() => {
              setGcalLoading(true);
              const tg = (window as any).Telegram?.WebApp;
              if (tg?.openTelegramLink) tg.openTelegramLink("https://t.me/aiplannerrubot?start=gcal_disconnect");
              setTimeout(() => { setGcalConnected(false); setGcalLoading(false); }, 1000);
            }}
            style={{
          ...
          <button
            onClick={() => {
              const tg = (window as any).Telegram?.WebApp;
              if (tg?.openTelegramLink) tg.openTelegramLink("https://t.me/aiplannerrubot?start=gcal");
              else window.open("https://t.me/aiplannerrubot?start=gcal", "_blank");
            }}
            style={{
              width: "100%", height: "40px", borderRadius: "10px",
              border: "none", backgroundColor: "#4285f4",
          ...
          >
            ...
            {ru ? "Подключить Google Calendar" : "Connect Google Calendar"}
          </button>
        )}
      </div>

      {/* ==================== КОМАНДНЫЕ ЗАДАЧИ ==================== */}
      <SectionTitle>👥 {ru ? "Командные задачи" : "Team Tasks"}</SectionTitle>
      <div
        style={{
          borderRadius: "14px",
          border: "1px solid rgba(255,255,255,0.08)",
      ...
      >
        <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", margin: "0 0 10px 0", lineHeight: "1.5" }}>
          {ru
            ? "Добавь бота в групповой чат для совместной работы над задачами. Участники могут создавать и назначать задачи друг другу."
            : "Add the bot to a group chat for collaborative task management. Members can create and assign tasks to each other."}
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
            <span style={{ fontSize: "13px", color: theme.primary, flexShrink: 0 }}>📋</span>
            <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", margin: 0 }}>
              /grouptask {ru ? "Подготовить отчёт @user в пятницу" : "Prepare report @user on Friday"}
            </p>
          </div>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "8px" }}>
            <span style={{ fontSize: "13px", color: theme.primary, flexShrink: 0 }}>⚡</span>
            <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", margin: 0 }}>
              @cortexaibot {ru ? "задача — создать из любого чата" : "task — create from any chat"}
            </p>
          </div>
        </div>
        <button
          ...
          style={{
            ...
            border: `1px solid ${theme.primary}40`, backgroundColor: `${theme.primary}15`,
            color: theme.primary, fontSize: "13px", fontWeight: 600, cursor: "pointer",
          }}
        >
