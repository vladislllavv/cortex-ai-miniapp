...
// Сброс ежедневных задач при старте
...
export const useTaskStore = create<TaskStore>((set) => ({
  ...
  // loadUserData + cleanup в одном запросе
...
export function usePersistTasks() {
  useEffect(() => {
    ...
    // Один вызов — cleanup внутри loadUserData
...
// Сброс ежедневных задач — вызывается при старте
function resetDailyTasks(tasks: Task[]): Task[] {
  const todayStr = new Date().toISOString().split("T")[0];
  return tasks.map((task) => {
    if (task.repeat !== "daily" || task.status !== "done") return task;
...
export const useTaskStore = create<TaskStore>((set, get) => ({
  ...
  // Объединяем loadUserData + cleanup в одном запросе
  loadUserData: async (userId) => {
    if (userId === "unknown") {
      set({ isDataLoaded: true });
      return;
    ...
    try {
      ...
      // Один проход — строим Map и удаляем устаревшие
      const cloudMap = new Map<string, Task>();
      const batch = writeBatch(db);
      ...
      tasksSnap.forEach((d) => {
        ...
        // Для daily задач — сбрасываем статус вместо удаления
        if (data.repeat === "daily" && data.status === "done" && data.completedAt) {
          const completedDay = data.completedAt.split("T")[0];
          if (completedDay < todayStr) {
            const resetTask = normalizeTask({
    ...
    const unsubTasks = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => {
          ...
          // Пропускаем выполненные обычные задачи старше 1 дня
          if (data.status === "done" && data.completedAt && data.repeat !== "daily") {
            if (new Date(data.completedAt) < yesterday) return;
          }

          // Daily задачи сбрасываем
          if (data.repeat === "daily" && data.status === "done" && data.completedAt) {
            const completedDay = data.completedAt.split("T")[0];
            if (completedDay < todayStr) {
              cloudMap.set(d.id, normalizeTask({
