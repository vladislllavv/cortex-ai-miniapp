...
// Варианты напоминания заранее (в минутах)
...
export default function AddBtn() {
  ...
  const handleClose = () => { setOpen(false); setTimeout(resetForm, 200); };
    ...
    try {
      ...
      if (todayTasks.length >= 5) {
        ...
        if (!hasSub) {
          ...
          tg?.showAlert(ru ? "Лимит 5 задач в день 📋\n\nОформи подписку." : "Daily limit 5 tasks.\n\nGet subscription.");
      ...
      if (date && time) {
        ...
        if (!isNaN(p.getTime())) {
          // Применяем смещение "напомнить заранее"
          const reminded = new Date(p.getTime() - remindBefore * 60 * 1000);
          dueDate = reminded.toISOString();
        }
      } else if (date) {
      ...
      await addTask({
        ...
        status: "todo",
        isAiCreated: false,
        repeat: (repeatValue === "none" ? "none" : repeatValue === "daily" ? "daily" : repeatValue) as any,
        type: taskType,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, backgroundColor: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}
      onClick={handleClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: "340px", backgroundColor: "#1e293b", borderRadius: "20px", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ padding: "20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
            <div style={{ display: "flex", gap: "4px" }}>
              {[0, 1, 2].map((s) => (
                <div key={s} style={{ height: "3px", width: "20px", borderRadius: "2px", backgroundColor: step >= s ? "#3b82f6" : "rgba(255,255,255,0.15)", transition: "background-color 0.3s" }} />
              ))}
            </div>
            {step > 0 && (
          ...
          {step === 1 && (
            <div>
              ...
              {taskType === "task" ? (
                <>
                  <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleNext()} placeholder={ru ? "Название задачи..." : "Task title..."} style={{ display: "block", width: "100%", boxSizing: "border-box" as const, height: "44px", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.07)", paddingLeft: "14px", paddingRight: "14px", fontSize: "15px", color: "white", outline: "none", marginBottom: "12px", fontFamily: "inherit" }} />

                  <p style={{ fontSize: "12px", fontWeight: 500, color: "rgba(255,255,255,0.4)", marginBottom: "8px" }}>{ru ? "Приоритет" : "Priority"}</p>
                  ...
                  <div style={{ maxHeight: "200px", overflowY: "auto", marginBottom: "10px" }}>
                    {shoppingItems.map((item, idx) => (
                      <div key={idx} style={{ display: "flex", gap: "6px", marginBottom: "8px" }}>
                        <input autoFocus={idx === 0} value={item} onChange={(e) => { const u = [...shoppingItems]; u[idx] = e.target.value; setShoppingItems(u); }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); setShoppingItems([...shoppingItems, ""]); } }} placeholder={ru ? `Товар ${idx + 1}...` : `Item ${idx + 1}...`} style={{ flex: 1, height: "42px", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.07)", paddingLeft: "12px", paddingRight: "12px", fontSize: "14px", color: "white", outline: "none", fontFamily: "inherit", boxSizing: "border-box" as const }} />
          ...
          {step === 2 && (
            <div>
              ...
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} style={{ display: "block", width: "100%", boxSizing: "border-box" as const, height: "44px", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.07)", paddingLeft: "14px", paddingRight: "14px", fontSize: "15px", color: "white", outline: "none", marginBottom: "12px", fontFamily: "inherit", colorScheme: "dark", appearance: "none" as any }} />
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "12px" }}>
                    {REMIND_BEFORE_OPTIONS.map((opt) => (
                      <button key={opt.value} type="button" onClick={() => setRemindBefore(opt.value)}
                        style={{ padding: "5px 10px", borderRadius: "10px", border: remindBefore === opt.value ? "1px solid #3b82f6" : "1px solid rgba(255,255,255,0.08)", backgroundColor: remindBefore === opt.value ? "rgba(59,130,246,0.2)" : "rgba(255,255,255,0.05)", color: remindBefore === opt.value ? "#60a5fa" : "rgba(255,255,255,0.5)", fontSize: "11px", cursor: "pointer", flexShrink: 0 }}>
              ...
              {/* Повтор */}
              <p style={{ fontSize: "12px", fontWeight: 500, color: "rgba(255,255,255,0.4)", marginBottom: "6px" }}>{ru ? "Повтор" : "Repeat"}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: repeat === "custom" ? "8px" : "16px" }}>
                {REPEAT_OPTIONS.map((opt) => (
                  <button key={opt.value} type="button" onClick={() => setRepeat(opt.value)}
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderRadius: "10px", border: repeat === opt.value ? "1px solid #3b82f6" : "1px solid rgba(255,255,255,0.08)", backgroundColor: repeat === opt.value ? "rgba(59,130,246,0.15)" : "rgba(255,255,255,0.04)", cursor: "pointer", width: "100%" }}>
                    <span style={{ fontSize: "13px", color: repeat === opt.value ? "#60a5fa" : "rgba(255,255,255,0.7)" }}>
                      {ru ? opt.label : opt.labelEn}
                    </span>
                    {repeat === opt.value && (
                      <div style={{ width: "16px", height: "16px", borderRadius: "50%", backgroundColor: "#3b82f6", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              {repeat === "custom" && (
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <div style={{ display: "flex", gap: "6px", justifyContent: "space-between" }}>
                    {WEEKDAYS.map((day) => (
                      <button key={day.key} type="button" onClick={() => toggleCustomDay(day.key)}
                        style={{ width: "40px", height: "40px", borderRadius: "50%", border: customDays.includes(day.key) ? "1px solid #3b82f6" : "1px solid rgba(255,255,255,0.08)", backgroundColor: customDays.includes(day.key) ? "rgba(59,130,246,0.3)" : "rgba(255,255,255,0.04)", color: customDays.includes(day.key) ? "#60a5fa" : "rgba(255,255,255,0.5)", fontSize: "11px", fontWeight: 600, cursor: "pointer", flexShrink: 0 }}>
                        {ru ? day.ru : day.en}
                      </button>
                    ))}
                  </div>
