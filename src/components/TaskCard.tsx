...
function getDateColor(isoString: string): string {
  try {
    const date = new Date(isoString);
    const now = new Date();
    ...
    if (date.toDateString() === now.toDateString()) return "#60a5fa";
...
export default memo(function TaskCard({ task }: { task: Task }) {
  ...
  const dateColor = task.dueDate && !isDone ? getDateColor(task.dueDate) : "rgba(255,255,255,0.4)";
  ...
  return (
    <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            {timeStr && (
              <div style={{ display: "flex", alignItems: "center", gap: "3px" }}>
                <Clock size={11} color={dateColor} />
                <span style={{ fontSize: "11px", color: dateColor }}>{timeStr}</span>
              </div>
            )}
            ...
            {isShopping && items.length > 0 && (
              <span style={{ fontSize: "11px", color: "rgba(34,197,94,0.7)" }}>
                {checkedCount}/{items.length} {ru ? "куплено" : "bought"}
              </span>
            )}

            <div style={{
              width: "6px", height: "6px", borderRadius: "50%",
              backgroundColor: getPriorityColor(task.priority), flexShrink: 0,
            }} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "2px", flexShrink: 0 }}>
          {!isDone && (
            <button
              ...
              style={{ background: "none", border: "none", cursor: "pointer", padding: "4px", display: "flex", alignItems: "center", opacity: 0.4, transition: "opacity 0.2s" }}
      ...
      {/* Панель переноса */}
      {showReschedule && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "10px 14px", backgroundColor: "rgba(59,130,246,0.05)" }}>
          ...
          <div style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
            <input
              ...
              style={{ flex: 1, minWidth: "130px", height: "36px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.07)", paddingLeft: "10px", paddingRight: "10px", fontSize: "13px", color: "white", outline: "none", fontFamily: "inherit", colorScheme: "dark", boxSizing: "border-box" as const }}
            />
            <input
              type="time"
              value={newTime}
              onChange={(e) => setNewTime(e.target.value)}
              style={{ width: "100px", minWidth: "100px", height: "36px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.07)", paddingLeft: "10px", paddingRight: "10px", fontSize: "13px", color: "white", outline: "none", fontFamily: "inherit", colorScheme: "dark", boxSizing: "border-box" as const }}
            />
            <button
              onClick={handleReschedule}
              disabled={!newDate}
              style={{ height: "36px", paddingLeft: "12px", paddingRight: "12px", borderRadius: "8px", border: "none", backgroundColor: newDate ? "#3b82f6" : "rgba(255,255,255,0.1)", color: "white", fontSize: "12px", fontWeight: 600, cursor: newDate ? "pointer" : "default", flexShrink: 0, display: "flex", alignItems: "center", gap: "4px" }}
            >
              <Check size={14} />
              OK
            </button>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "8px 14px 12px 14px" }}>
          {items.map((item, idx) => (
            <div
              ...
              style={{ display: "flex", alignItems: "center", gap: "10px", padding: "6px 0", cursor: "pointer", borderBottom: idx < items.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}
            >
              <div style={{
